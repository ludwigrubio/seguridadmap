(function() {
    ...
    HomeController.$inject = ['$scope', 'Principal', 'LoginService', '$state','olData','Geoserver','GeoLayers','GeoCapabilities','$http','$filter','ConfigStore','WMS_URL','BASE_DOMAIN','BASE_DOMAIN_TEXT'];

    function HomeController ($scope, Principal, LoginService, $state,olData,Geoserver,GeoLayers,GeoCapabilities,$http,$filter,ConfigStore,WMS_URL,BASE_DOMAIN,BASE_DOMAIN_TEXT) {
        /**
        *  V A R I A B L E S
        * */
        /********************************************************************************************************/
        ...
        vm.sourceFunctionFiscalia = function() {
					var label = this.renderer.text("<p>La información puede modificarse en base a los resultados de la investigación, " +
					"puede sufrir cambio en el tipo de delito o pudieran presentarse delitos adicionales, " +
					"por lo que debe de ser considerada la información con las reservas a estas aclaraciones. </p><br> " +
					"<p>La información contenida en el gráfico, hace referencia a los eventos que cuentan con los datos suficientes para ser georreferenciados.</p><br>" +
					"<strong>Fuente:</strong> Fiscalía General del Estado de Jalisco.")
	        .css({
	            width: '450px',
	            fontSize: '5px'
	        })
          ...
          label.align(Highcharts.extend(label.getBBox(), {
            ...
            var labelGeo = this.renderer.text("<div>" +
                "<a href='" + BASE_DOMAIN + "'>" +
                BASE_DOMAIN +
                "</a>" +
                "</div>")
                ...
                .attr({ useHTML:true })
                .add();
            labelGeo.align(Highcharts.extend(label.getBBox(), {
                x: 0, // offset
                align: 'left',
                verticalAlign: 'top',
                y: 0 // offset
        ...
        vm.filters.style_marginacion={
            ...
            default_colonias: {style:'mapa_jalisco:default_colonias'},
            default_agebs: {style:'mapa_jalisco:default_agebs'},
            default_municipios: {style:'mapa_jalisco:default_municipios'},
        ...
        olData.getMap("main1").then(function (oMap) {
            ...
            $scope.downloadDelitosAnio = function(){
               var header = '<meta http-equiv="content-type" content="application/vnd.ms-excel; charset=UTF-8">';
	             var blob = new Blob([ header + document.getElementById('table-delitos-anio').innerHTML.replace("</caption>","<strong>Fuente:</strong> Fiscalía General del Estado de Jalisco.</caption>")], {
            ...
            $scope.downloadDelitosTipo = function(){
                var header = '<meta http-equiv="content-type" content="application/vnd.ms-excel; charset=UTF-8">';
                var blob = new Blob([ header + document.getElementById('table-delitos-tipo').innerHTML.replace("</caption>","<strong>Fuente:</strong> Fiscalía General del Estado de Jalisco.</caption>")], {
        ...
        * */
        ...
        $scope.$watch('vm.filters.style_marginacion.agebs.status', function(status) {
            if(status)
                ...
                agebs.getSource().updateParams({"STYLES":  vm.filters.style_marginacion.default_agebs.style });
        });
        $scope.$watch('vm.filters.style_marginacion.colonias.status', function(status) {
            if(status)
                ...
                colonias.getSource().updateParams({"STYLES":  vm.filters.style_marginacion.default_colonias.style });
        });
       ...
       // imMun();
                            ...
                            chartOptions:{
                                ...
                                chart: {
												            ...
												            load: function () {
																			...
																			"<strong>Fuente:</strong> CONAPO 2015.")
															        .css({
															            width: '450px',
															            fontSize: '5px',
															        })
                        ...
                        exporting:{
                            chartOptions:{
                                ...
                                chart: {
												            ...
												            load: function () {
																			var label = this.renderer.text(
																			"<strong>Fuente:</strong> CONAPO 2010.")
															        .css({
															            width: '450px',
															            fontSize: '5px',
															        })
                   ...
                   vm.totalDelitos=0;
                    ...
                    $.each(res.AggregationResults.GroupByResult['object-array'], function (index, value) {
                        ...
                        if(value.int!=undefined){
                            ...
                            vm.totalDelitos+=parseInt(value.int[1]);
                        }else if(value[0]!=undefined){
                            obj.name = value[0];
                            obj.y = parseFloat(value[1]);
                            vm.delitoAnio.push(obj);
                            vm.totalDelitos+=parseInt(value[1]);
                        }
                            ...
                            options: {
                                ...
                                exporting: {
                                  ...
                                  chartOptions: {
                                      ...
                                      subtitle: {
                                          text: $( '#description-delitos-anio' ).html(),
                                          style: {
                                              color: '#666',
                                              fontSize: '8px'
                                          }
                                      ...
                                      chart: {
		                                    marginBottom: 100,
                                        events: {
                                                    load: vm.sourceFunctionFiscalia
                    ...
                    options: {
                        ...
                        exporting:{
                            chartOptions:{
                                ...
                                subtitle: {
                                    text: $( '#description-delitos-tipo' ).html(),
                                    style: {
                                        color: '#666',
                                        fontSize: '8px'
                                    }
        ...
        $scope.$watchGroup(['vm.filters.region', 'vm.filters.municipios',
            ...
            'vm.cql.map.gmPosList','vm.filters.type_filter_year'], function(newValue) {
