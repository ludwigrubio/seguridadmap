...
public class UserResource {
    ...
    public ResponseEntity<ManagedUserVM> updateUser(@RequestBody ManagedUserVM managedUserVM) {
        ...
        userService.updateUser(managedUserVM.getId(), managedUserVM.getLogin(), managedUserVM.getFirstName(),
            ...
            managedUserVM.getLangKey(), managedUserVM.getAuthorities(), managedUserVM.getImageProfile());
    ...
    public ResponseEntity<Void> deleteUser(@PathVariable String login) {
       ...
       //Added to delete file realationed
        if (user.isPresent()) {
            Boolean activated = false;

            if (user.get().getActivationKey() != "") {
                activated = true;
            } else {
                activated = false;
            }

            Set<String> userToDelete = new HashSet<String>();
            userToDelete.add("ROLE_USER");

            log.debug("REST request to delete User : {}", login);

            userService.updateUser(user.get().getId(), user.get().getLogin(), user.get().getFirstName(),
                user.get().getLastName(), user.get().getEmail(), activated,
                user.get().getLangKey(), userToDelete, null);

            if (user.get().getImageProfile() != null) {
                File fileToDelete = fileRepository.getOne(user.get().getImageProfile().getId());
